...
# 添加 triton/chunk_loss 路径以导入 ref_indexer_loss_fwd_interface
sys.path.insert(0, os.path.join(os.path.dirname(__file__), '../../triton/chunk_loss'))
...
def flash_mla_to_attn_sum(
    ...
    计算公式:
    - p_out = exp2(qk * scale - max_logits)
    - lse = log2(sum of exp2) + max_logits
    ...
    Args:
        ...
        max_logits: [s_q, h_q] - 最大 logits（已乘以 scale）
        lse: [s_q, h_q] - log-sum-exp（log2 空间）
    
    Returns:
        attn_sum: [s_q, topk] - 所有 head 的 attention 概率求和
    """
    # 计算 sum = exp2(lse - max_logits)
    sum_exp = torch.exp2(lse - max_logits)  # [s_q, h_q]
    
    # 计算 attn_prob = p_out / sum
    attn_prob = p_out / sum_exp.unsqueeze(-1)  # [s_q, h_q, topk]
    
    # 计算 attn_sum = sum over heads
...
def run_accuracy_test(config: TestConfig, device: str = 'cuda') -> dict:
    ...
    # 后处理得到 attn_sum
    flash_attn_sum = flash_mla_to_attn_sum(p_out, max_logits, lse)
    ...
    abs_diff = torch.abs(flash_attn_sum_f32 - ref_attn_sum_f32)
    max_diff = abs_diff.max().item()
    rel_diff = (abs_diff / (1e-6 + torch.abs(ref_attn_sum_f32))).mean().item() * 100
...
def run_accuracy_test_vs_tilelang(config: TestConfig, device: str = 'cuda') -> dict:
    ...
    # 尝试导入 tilelang 的参考实现
    try:
        from indexer_loss_kernel_tilelang import ref_indexer_loss_fwd_interface
    ...
    # TileLang 参考实现
    tl_attn_sum = ref_indexer_loss_fwd_interface(q_tl, kv_tl, indices_tl, chunk_offset, kv_stride, sm_scale)
...
if __name__ == "__main__":
    ...
    # 精度测试配置
    accuracy_configs = [
        TestConfig(name="小规模", num_heads=128, chunk_size=256, seq_len=512, topk=128),
        TestConfig(name="中等规模", num_heads=128, chunk_size=512, seq_len=1024, topk=256),
        TestConfig(name="大规模", num_heads=128, chunk_size=1024, seq_len=2048, topk=512),
        TestConfig(name="大topk", num_heads=128, chunk_size=512, seq_len=4096, topk=1024),
        TestConfig(name="长序列", num_heads=128, chunk_size=2048, seq_len=8192, topk=2048),
    ]
