...
def pytorch_reference_loss(q, kv, index_score, indices, chunk_offset, kv_stride=1, sm_scale=None):
    """
    PyTorch 参考实现: 计算 KL 散度 loss
    
    Args:
        q: [batch, chunk_size, heads, dim + tail_dim]
        kv: [batch, seq_len_kv, kv_group, dim + tail_dim]
        index_score: [batch, chunk_size, topk] - 需要梯度
    ...
    index_score_masked = index_score.masked_fill(causal_mask, float('-inf'))
    index_prob = torch.softmax(index_score_masked, dim=-1) + eps
    
    # KL 散度: KL(attn_dist || index_prob)
    kl_loss = F.kl_div(index_prob.log(), attn_dist.to(index_prob.dtype) + eps, reduction='batchmean')
    
    return kl_loss


def pytorch_reference_bwd(q, kv, index_score, indices, chunk_offset, kv_stride=1, sm_scale=None):
    """
    PyTorch 参考实现: 使用 autograd 计算反向传播
    
    先计算前向 loss，然后调用 loss.backward() 获取梯度。
    ...
    Returns:
        grad_index_score: [batch, chunk_size, topk] - 梯度
    """
    if not index_score.requires_grad:
        index_score = index_score.detach().clone().requires_grad_(True)
    
    # 计算前向 loss
    loss = pytorch_reference_loss(q, kv, index_score, indices, chunk_offset, kv_stride, sm_scale)
    
    # 反向传播获取梯度
    loss.backward()
    
    return index_score.grad
...
def run_bwd_accuracy_test(config: TestConfig, device: str = 'cuda'):
    ...
    # PyTorch 参考: 使用 autograd 计算梯度 (先算 loss 再 backward)
    index_score_ref = index_score.clone().requires_grad_(True)
    ref_grad = pytorch_reference_bwd(q, kv, index_score_ref, indices, chunk_offset, kv_stride)
    
    # Triton 版本: 使用自定义 backward
    index_score_tri = index_score.clone().requires_grad_(True)
    loss = indexer_loss_opt(q, kv, index_score_tri, indices, chunk_offset, kv_stride)
    loss.backward()
    tri_grad = index_score_tri.grad
