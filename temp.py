...
def _attention_softmax_kernel_v2(
    ...
    """
    高性能Attention Softmax Kernel
    
    优化:
    1. 每个program处理BLOCK_M行query
    2. 使用2-pass online softmax: 
       - Pass 1: 遍历K，计算max和sum，存储exp(qk-max)到output
       - Pass 2: 用sum归一化output (原地除法)
    3. 使用tl.dot进行批量矩阵乘法
    """
    pid = tl.program_id(0)
    ...
    # 临时存储: 需要存储exp(qk - m_i)，但由于m_i会变化，需要rescale
    # 我们先计算所有的max和sum，然后第二遍写出
    ...
    for start_n in range(0, seq_len, BLOCK_N):
        ...
        mask_load = (offs_m[:, None] < seq_len) & n_mask[None, :]
        mask_val = tl.load(mask_ptrs, mask=mask_load, other=True)
        qk = tl.where(mask_val, -float("inf"), qk)
    ...
    for start_n in range(0, seq_len, BLOCK_N):
        ...
        out_mask = (offs_m[:, None] < seq_len) & n_mask[None, :]
...
def attention_softmax_fused(query, key, mask, scaling):
    ...
    # 调整BLOCK_M和BLOCK_N以适应不同的seq_len
    if seq_len <= 512:
        BLOCK_M = 32
        BLOCK_N = 32
    elif seq_len <= 2048:
