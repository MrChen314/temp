...
def get_autotune_configs():
    """生成autotune配置，针对H20 (Hopper) GPU"""
    configs = []
    # H20推荐配置: num_warps=4-8, num_stages=2-4
...
@triton.autotune(
    configs=[
        ...
        triton.Config({'BLOCK_M': 4, 'BLOCK_D': 128}, num_stages=2, num_warps=4),
        triton.Config({'BLOCK_M': 8, 'BLOCK_D': 64}, num_stages=2, num_warps=4),
        triton.Config({'BLOCK_M': 8, 'BLOCK_D': 128}, num_stages=2, num_warps=8),
        triton.Config({'BLOCK_M': 16, 'BLOCK_D': 64}, num_stages=2, num_warps=8),
        triton.Config({'BLOCK_M': 16, 'BLOCK_D': 128}, num_stages=3, num_warps=8),
    ],
...
def _sparse_attn_multirow_kernel(
    ...
    """
    多行处理的Sparse Attention Kernel
    
    每个program处理BLOCK_M行query，减少kernel启动开销
    """
    pid = tl.program_id(0)
