    ...
    if (warpgroup_idx == 0) {
        ...
        for (int k = 0; k < num_k_blocks; ++k) {
            ...
            if (params.p_out != nullptr) {
                ...
                // Use float4 vectorized write for efficiency (64 floats = 16 x float4)
                float4* p_out_ptr4 = (float4*)p_out_ptr;
                float4* p_float4 = (float4*)p_float;
                CUTE_UNROLL
                for (int i = 0; i < (B_TOPK/2)/4; i++) {
                    p_out_ptr4[i] = p_float4[i];
                }
            }
