...
// AttnDistParams: Parameters for computing attention distribution using pre-computed LSE
...
struct AttnDistParams {
    ...
    // Input tensors
    cutlass::bfloat16_t* __restrict__ q;      // [s_q, h_q, d_qk]
    cutlass::bfloat16_t* __restrict__ k;      // [s_kv, h_kv, d_qk]
    int* __restrict__ indices;                 // [s_q, h_kv, topk]
    float* __restrict__ lse;                   // [s_q, h_q] - Pre-computed LSE (log2-based)
    ...
    // Output tensor
    float* __restrict__ attn_sum;              // [h_q/64, s_q, topk] - each head group (64 heads) outputs separately
