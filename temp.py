...
def sparse_mla_fwd_opt2_reduced_sync(
    ...
    def main(
        ...
        with T.Kernel(seq_len * REPLICATE_H, batch, kv_group, threads=threads) as (bx, by, bz):
            ...
            m_i_prev = T.alloc_fragment([H_per_block], accum_dtype)
            # 用于存储 softmax 输出
            S_local = T.alloc_fragment([H_per_block, BI], dtype)
            ...
            for i_i in T.Pipelined(NI, num_stages=num_stages):
                ...
                # 优化2: 直接转换类型，减少一次 shared memory 写入
                for h_i, bi_i in T.Parallel(H_per_block, BI):
                    S_local[h_i, bi_i] = acc_s[h_i, bi_i]
                T.gemm(S_local, KV_shared, acc_o, policy=T.GemmWarpPolicy.FullRow)
...
def sparse_mla_fwd_opt3_preload_indices(
    ...
    """优化3: 预加载 Indices 到 shared memory，减少每次迭代的 global memory 访问"""
    assert dim == tilelang.math.next_power_of_2(dim)
    assert tail_dim == tilelang.math.next_power_of_2(tail_dim)
    assert is_causal == True
    assert topk % block_I == 0
    ...
    def main(
        ...
        with T.Kernel(seq_len * REPLICATE_H, batch, kv_group, threads=threads) as (bx, by, bz):
            ...
            # 用于存储当前 block 的 index
            idx_local = T.alloc_local([1], indices_dtype)
            ...
            for i_i in T.Pipelined(NI, num_stages=num_stages):
                # 优化3: 从 shared memory 读取 indices
                for bi_i in T.Parallel(BI):
                    idx_local[0] = Indices_shared[i_i * BI + bi_i]
                    mask[bi_i] = idx_local[0] <= max_kv_i
...
def sparse_mla_fwd_opt4_small_h(
    heads,
    dim,
    tail_dim,
    topk,
    ...
    """
    优化4: 针对小 H 的特化
    当 H <= 4 时，使用更紧凑的 H_per_block 避免无效计算
    原始: H=2, padded_H=16, 有效计算仅 2/16 = 12.5%
    优化: H=2, H_per_block=2, 有效计算 100%
    """
    assert dim == tilelang.math.next_power_of_2(dim)
    assert tail_dim == tilelang.math.next_power_of_2(tail_dim)
    assert is_causal == True
    ...
    H = head_kv
    # 优化4: 针对小 H 的特化，不做强制 padding 到 16
    if H <= 4:
        # 使用实际的 H 值，但需要至少 padded 到 2 的幂次
        padded_H = max(tilelang.math.next_power_of_2(head_kv), 2)
    else:
        padded_H = max(tilelang.math.next_power_of_2(head_kv), 16)
...
def benchmark_comparison(
    ...
    # 定义所有优化方案
    optimizations = [
        ("原始方案 (优化1配置)", sparse_mla_fwd_baseline),
        ("优化2: 减少同步开销", sparse_mla_fwd_opt2_reduced_sync),
        ("优化3: 预加载 Indices", sparse_mla_fwd_opt3_preload_indices),
        ("优化4: 小 H 特化", sparse_mla_fwd_opt4_small_h),
        ("综合优化 (2+3+4)", sparse_mla_fwd_combined),
    ]
    
    results = []
    ref_out = None
    
    for name, kernel_func in optimizations:
        print(f"\n测试: {name}")
        print("-" * 40)
