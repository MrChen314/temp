...
def sparse_mla_fwd(
    ...
    m_shape = [batch, seq_len, heads]  # M_out shape
    ...
    def main(
        ...
        with T.Kernel(seq_len * REPLICATE_H, batch, kv_group, threads=threads) as (
            ...
            # Save M_out (m_i * sm_scale, same as used in Lse calculation)
            m_out_shared = T.alloc_shared([H_per_block], accum_dtype)
            for h_i in T.Parallel(H_per_block):
                m_out_shared[h_i] = m_i[h_i] * sm_scale

            T.copy(acc_o, O_shared)
            T.copy(acc_o, Output[b_i, s_i, H0:H1, :])
            T.copy(sumexp, Lse_shared)
            T.copy(sumexp, Lse[b_i, s_i, H0:H1])
            T.copy(m_out_shared, M_out[b_i, s_i, H0:H1])

    return main


def sparse_mla_fwd_interface(q, kv, indices, sm_scale=None, return_p_sum: bool = False, d_v=512, block_I=64, num_stages=2, threads=256, return_m_out=False):
    is_casual = True
    ...
    out, lse, m_out = kernel(q, kv, indices)
    if return_m_out:
        return out, lse, m_out
    return out, lse
