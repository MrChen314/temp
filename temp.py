...
# ============================================================================
# 优化8: 批量处理 query - 无约束版本（改进版）
# 
# 原理说明:
# - 保持 topk 不变（不取并集，避免计算量暴增）
# - 使用 block 内最后一个 seq position 的 indices（和优化7一样）
# - 预计算精确的 mask：检查每个 kv_idx 是否在各个 seq position 的原始 indices 中
# 
# 优点:
# - 无 seq_len 必须是 S_per_block 倍数的约束
# - 计算量与原始相当（topk 不变）
# - mask 精确标记哪些 KV 应该被哪个 seq position 访问
# 
# 关键洞察:
# - 虽然我们使用最后一个 seq position 的 indices 加载 KV
# - 但 mask 会精确屏蔽不属于某个 seq position 原始 indices 的 KV
# - 这样每个 seq position 只会 attend 到它原本应该访问的 KV
# ============================================================================

def prepare_batched_mask(indices, S_per_block, is_causal=True):
    """
    预处理 mask，为批量处理 query 准备精确的 mask
    
    不改变 indices，只预计算 mask：
    - 使用 block 内最后一个 seq position 的 indices
    - mask[b, blk, g, s_offset, k] = True 当且仅当:
      1. indices[b, s_last, g, k] 在 indices[b, s_base+s_offset, g, :] 中
      2. (causal) indices[b, s_last, g, k] <= s_base + s_offset
    ...
    Returns:
        mask: [batch, num_blocks, kv_group, S_per_block, topk]
    """
    ...
    # 创建 mask
    mask = torch.zeros(
        (batch, num_blocks, kv_group, S_per_block, topk),
    ...
    for b in range(batch):
        for blk in range(num_blocks):
            ...
            # 使用 block 内最后一个有效 seq position 的 indices
            s_last = min(s_base + S_per_block - 1, seq_len - 1)
            
            for g in range(kv_group):
                # 最后一个 seq position 的 indices
                last_indices = indices[b, s_last, g, :].tolist()
                ...
                for s_offset in range(S_per_block):
                    ...
                    # 当前 seq position 的原始 indices
                    pos_indices_set = set(indices[b, s_i, g, :].tolist())
                    
                    for k, kv_idx in enumerate(last_indices):
                        # 条件1: kv_idx 在当前 seq position 的原始 indices 中
                        in_original = kv_idx in pos_indices_set
...
def prepare_batched_mask_fast(indices, S_per_block, is_causal=True):
    """
    GPU 加速版本：预计算精确的 mask
    ...
    if pad_len > 0:
        # Pad with a large invalid index
        indices_padded = torch.nn.functional.pad(
            indices, (0, 0, 0, 0, 0, pad_len), value=seq_len + 1000
        )
    else:
    ...
    # Permute: [batch, num_blocks, kv_group, S_per_block, topk]
    indices_block = indices_reshaped.permute(0, 1, 3, 2, 4)
    
    # 获取每个 block 最后一个 seq position 的 indices
    # last_indices: [batch, num_blocks, kv_group, topk]
    last_indices = indices_block[:, :, :, -1, :]  # 取 S_per_block-1 位置
    
    # 但要处理边界情况：如果 block 的最后一个位置超出 seq_len，使用 seq_len-1
    # 生成每个 block 最后一个有效位置的索引
    block_end = torch.arange(num_blocks, device=device) * S_per_block + (S_per_block - 1)
    block_end = torch.clamp(block_end, max=seq_len - 1)  # [num_blocks]
    
    # 重新获取 last_indices (需要 gather)
    # indices: [batch, seq_len, kv_group, topk]
    # 需要: [batch, num_blocks, kv_group, topk]
    block_end_expanded = block_end.view(1, num_blocks, 1, 1).expand(batch, -1, kv_group, topk)
    last_indices = torch.gather(indices, 1, block_end_expanded)  # [batch, num_blocks, kv_group, topk]
    
    # 现在计算 mask
    # 对于每个 (b, blk, g, s_offset, k)，检查 last_indices[b, blk, g, k] 是否在 
    # indices_block[b, blk, g, s_offset, :] 中
    
    # last_indices: [batch, num_blocks, kv_group, topk]
    # indices_block: [batch, num_blocks, kv_group, S_per_block, topk]
    
    # Expand last_indices: [batch, num_blocks, kv_group, 1, topk, 1]
    last_expanded = last_indices.unsqueeze(3).unsqueeze(-1)
    # Expand indices_block: [batch, num_blocks, kv_group, S_per_block, 1, topk]
    block_expanded = indices_block.unsqueeze(4)
    
    # 比较: [batch, num_blocks, kv_group, S_per_block, topk, topk]
    matches = (last_expanded == block_expanded)
    
    # 在最后一个维度 reduce: 只要 last_indices[k] 等于 block 中任意一个即可
    in_original = matches.any(dim=-1)  # [batch, num_blocks, kv_group, S_per_block, topk]
    ...
    if is_causal:
        ...
        # last_indices[k] <= s_i
        # s_i: [num_blocks, S_per_block] -> [1, num_blocks, 1, S_per_block, 1]
        s_i_expanded = s_i.view(1, num_blocks, 1, S_per_block, 1)
        # last_indices: [batch, num_blocks, kv_group, topk] -> [batch, num_blocks, kv_group, 1, topk]
        last_expanded = last_indices.unsqueeze(3)
        
        causal_mask = last_expanded <= s_i_expanded  # [batch, num_blocks, kv_group, S_per_block, topk]
        mask = mask & causal_mask
    
    # 处理无效的 seq positions (s_i >= seq_len)
    s_i_full = block_ids * S_per_block + s_offsets  # [num_blocks, S_per_block]
    valid_pos = s_i_full < seq_len  # [num_blocks, S_per_block]
    valid_pos = valid_pos.view(1, num_blocks, 1, S_per_block, 1).expand_as(mask)
    mask = mask & valid_pos
...
def sparse_mla_fwd_batched_query_v2(
    ...
    使用预处理好的 mask（topk 不变）:
    - Indices: [batch, seq_len, kv_group, topk] - 原始 indices
    - Mask: [batch, num_blocks, kv_group, S_per_block, topk] - 预计算的精确 mask
    
    每个 block 使用最后一个 seq position 的 indices 加载 KV，
    但 mask 精确标记哪些 KV 应该被哪个 seq position 访问
    """
    assert dim == tilelang.math.next_power_of_2(dim)
    assert tail_dim == tilelang.math.next_power_of_2(tail_dim)
    assert is_causal == True
    ...
    kv_shape = [batch, seq_len_kv, kv_group, dim + tail_dim]
    o_shape = [batch, seq_len, heads, dim]
    # 原始 indices 和预处理的 mask
    indices_shape = [batch, seq_len, kv_group, topk]
    mask_shape = [batch, num_blocks, kv_group, S_per_block, topk]
    lse_shape = [batch, seq_len, heads]
    indices_dtype = T.int32
    dtype = T.bfloat16
    accum_dtype = T.float32
...
def sparse_mla_fwd_batched_v2_wrapper(q, kv, indices, S_per_block=8, block_I=64, **kwargs):
    ...
    # 预处理 mask（topk 不变）
    mask = prepare_batched_mask_fast(indices, S_per_block, is_causal=True)
    ...
    # 运行 kernel
    out, lse = kernel(q, kv, indices, mask)
    ...
    for name, kernel_func, block_i, num_threads, s_per_block, use_v2 in optimizations:
        ...
        try:
            if use_v2:
                ...
                def fn():
                    return kernel(q, kv, indices, mask)
            else:
                # 编译 kernel
                if s_per_block > 1:
                    # 优化7: 批量处理
