...
def _sparse_attn_loss_fused_kernel(
    ...
    for h_block in range(num_head_blocks):
        ...
        for tk_idx in range(num_topk_blocks):
            ...
            # 应用 causal mask: 对被 mask 的位置设为 NEG_INF
            qk = tl.where(causal_mask_block[:, None], NEG_INF, qk)  # [BLOCK_TOPK, BLOCK_H]
        ...
        for tk_idx in range(num_topk_blocks):
            ...
            qk = qk * scaling
            qk = tl.where(causal_mask_block[:, None], NEG_INF, qk)
            
            # 使用全局 max/sum 归一化: [BLOCK_TOPK, BLOCK_H]
            p = tl.exp(qk - m_global[None, :]) / l_global[None, :]
            p = tl.where(causal_mask_block[:, None], 0.0, p)
            
            # 对所有 head 求和: [BLOCK_TOPK]
