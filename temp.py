...
def _sparse_attn_loss_fused_kernel(
    ...
    for h_block in range(num_head_blocks):
        ...
        for tk_idx in range(num_topk_blocks):
            ...
            for d_idx in range(num_d_blocks):
                ...
                qk = (qk + tl.dot(k_gathered, qT).to(tl.bfloat16)).to(tl.bfloat16)
            
            qk = (qk * scaling).to(tl.bfloat16)
            ...
            qk = tl.where(invalid_mask, NEG_INF, qk).to(tl.bfloat16)  # [BLOCK_TOPK, BLOCK_H]
            
            # Online softmax update - 对每个 head 独立
            m_block = tl.max(qk, axis=0).to(tl.bfloat16)  # [BLOCK_H]
            m_new = tl.maximum(m_global, m_block).to(tl.bfloat16)  # [BLOCK_H]
            # 修正旧的 sum，并加上新块的 exp
            # 关键修复: 在exp之后将无效位置显式设为0，避免 exp(NEG_INF - NEG_INF) = 1 的问题
            exp_qk = tl.exp(qk - m_new[None, :]).to(tl.bfloat16)
            exp_qk = tl.where(invalid_mask, 0.0, exp_qk).to(tl.bfloat16)
            l_global = (l_global * tl.exp(m_global - m_new).to(tl.bfloat16) + tl.sum(exp_qk, axis=0).to(tl.bfloat16)).to(tl.bfloat16)
            m_global = m_new
        ...
        for tk_idx in range(num_topk_blocks):
            ...
            invalid_mask = causal_mask_block[:, None] | (~h_mask[None, :])
            qk = tl.where(invalid_mask, NEG_INF, qk).to(tl.bfloat16)
            
            # 使用全局 max/sum 归一化: [BLOCK_TOPK, BLOCK_H]
            p = (tl.exp(qk - m_global[None, :]) / l_global[None, :]).to(tl.bfloat16)
            p = tl.where(invalid_mask, 0.0, p).to(tl.bfloat16)  # 对 padding head 和 causal masked 位置设为 0
    ...
    # =========================================================================
    is_m_global = tl.full([1], NEG_INF, dtype=tl.bfloat16)[0]
    is_l_global = tl.full([1], 0.0, dtype=tl.bfloat16)[0]
    ...
    for tk_idx in range(num_topk_blocks):
        ...
        is_m_new = tl.maximum(is_m_global, is_m_block).to(tl.bfloat16)
        # 关键修复: 在exp之后将无效位置显式设为0
        is_exp_val = tl.exp(is_val - is_m_new).to(tl.bfloat16)
        is_exp_val = tl.where(causal_mask_block, 0.0, is_exp_val).to(tl.bfloat16)
        is_l_global = (is_l_global * tl.exp(is_m_global - is_m_new).to(tl.bfloat16) + tl.sum(is_exp_val).to(tl.bfloat16)).to(tl.bfloat16)
    ...
    attn_total = tl.full([1], 0.0, dtype=tl.bfloat16)[0]
    for tk_idx in range(num_topk_blocks):
        tk_start = tk_idx * BLOCK_TOPK
        offs_tk = tk_start + tl.arange(0, BLOCK_TOPK)
        tk_mask = offs_tk < topk
        attn_sum_block = tl.load(attn_sum_base + offs_tk * stride_ask, mask=tk_mask, other=0.0).to(tl.bfloat16)
        attn_total = (attn_total + tl.sum(attn_sum_block).to(tl.bfloat16)).to(tl.bfloat16)
    
    attn_total = tl.where(attn_total < eps, 1.0, attn_total).to(tl.bfloat16)
    
    # 计算 KL 散度
    kl_sum = tl.full([1], 0.0, dtype=tl.bfloat16)[0]
    for tk_idx in range(num_topk_blocks):
        tk_start = tk_idx * BLOCK_TOPK
        offs_tk = tk_start + tl.arange(0, BLOCK_TOPK)
        tk_mask = offs_tk < topk
        ...
        kl = tl.where(causal_mask_block, 0.0, kl).to(tl.bfloat16)
        kl_sum = (kl_sum + tl.sum(kl).to(tl.bfloat16)).to(tl.bfloat16)
